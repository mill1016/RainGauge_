...
# --- Detect current location from browser automatically ---
...
if coords and coords.get("latitude") and coords.get("longitude"):
    ...
    st.warning("Could not detect location automatically. Please allow location access or enter your location manually.")
...
def get_precip_data(stations):
    ...
    API_KEY = "e1f10a1e78da46f5b10a1e78da96f525"
    end_date = date.today() - timedelta(days=1)
    start_date = end_date - timedelta(days=29)  # Always fetch last 30 days
    records = []
    ...
    for day in pd.date_range(start_date, end_date):
        ...
        for _, row in stations.iterrows():
            ...
            try:
                ...
                if not r.content:
                    #st.warning(f"No response data for {row['stationId']} on {date_str}")
                    continue
